...
type Props = { params: Promise<{ slug: string; categorySlug: string }>; searchParams: Promise<{ token?: string; preview?: string }> };
export default async function StoreCategory({ params, searchParams }: Props) {
  const [{ slug, categorySlug }, query] = await Promise.all([params, searchParams]);
  const storefront = await getStorefrontBySlug(slug, { token: query.token, previewToken: query.preview });
  if (!storefront) notFound();
  return <StorefrontPage kind="listing" storefront={storefront} categorySlug={decodeURIComponent(categorySlug)} />;
}
